...
def get_half_century_by_year(year):
    if 1800 < year <= 1850:
        return 0
    elif 1850 < year <= 1900:
        return 1
...
csv_filenames = [
    "all_char_en.csv", "all_word_en.csv", "all_rhythm_en.csv", "all_char_and_rhythm_en.csv", "all_char_and_word_en.csv", "all_word_and_rhythm_en.csv", "all_features_en.csv", "all_char_ru.csv", "all_word_ru.csv", "all_rhythm_ru.csv", "all_char_and_rhythm_ru.csv", "all_char_and_word_ru.csv", "all_word_and_rhythm_ru.csv", "all_features_ru.csv",
    "all_char_fr.csv", "all_word_fr.csv", "all_rhythm_fr.csv", "all_char_and_rhythm_fr.csv", "all_char_and_word_fr.csv", "all_word_and_rhythm_fr.csv",
    "all_features_fr.csv"]
result_file = open('net_century.txt', "w")
for filename in csv_filenames:
    ...
    precision = precision_score(np.argmax(y_test, axis=1), np.argmax(predictions, axis=1), average='macro') * 100
    recall = recall_score(np.argmax(y_test, axis=1), np.argmax(predictions, axis=1), average='macro') * 100
