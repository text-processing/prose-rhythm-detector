...
from models.aspect_finder.utils.conjunctions import conjunctions
...
def __parse_diacope_inside_chapter(chapter: list, start_count: int, excluded_words: set) -> list:
    ...
    :param chapter: chapter to find diacope from (as list of sentences)
...
def find(document) -> list:
    """
    Finds diacope in the specified document

    :param document: document to find diacope in
    :return: list with diacope (Feature objects)
    """
    stop_words = set(reduce(add, document.stop_words['diacope'].values())) if document.stop_words['diacope'] else set()
    excluded_words = set(conjunctions(document.lang)) | stop_words
    res = []
    words_by_sent_by_chapter = [[[word.lower() for word in sentence.words_list] for sentence in chapter]
